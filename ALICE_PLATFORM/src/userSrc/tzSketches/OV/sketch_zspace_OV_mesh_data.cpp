...
string folderPath = "data/USD/panels/";
//string filePath = "omniverse://nucleus.zaha-hadid.com/Projects/1453_CODE/UserTests/TZ/testCube_1.usda";
//string filePath_out = "omniverse://nucleus.zaha-hadid.com/Projects/1453_CODE/UserTests/TZ/testRoof.usda";
string filePath_out = "omniverse://nucleus.zaha-hadid.com/Projects/1453_CODE/UserTests/TZ/testRoof_clusters.usda";

//string filePath_out = "omniverse://nucleus.zaha-hadid.com/Projects/1453_CODE/UserTests/TZ/testCube_out.usda";
//string filePath_out = "C:/Users/Taizhong.Chen/Downloads/testCube_out.usda";
...
void toUSDFile(UsdGeomMesh meshPrim, zObjMesh& oMesh)
{
	...
	//create and set mesh attributes
	meshPrim.CreateFaceVertexCountsAttr().Set(fVCounts);
	meshPrim.CreateFaceVertexIndicesAttr().Set(fVIDs);
	meshPrim.CreateNormalsAttr().Set(normals);
	meshPrim.CreatePointsAttr().Set(points);
	meshPrim.CreateOrientationAttr().Set(TfToken("rightHanded"));
	meshPrim.CreateDisplayColorAttr().Set(vCols);

	//auto colPrimvar = meshPrim.CreateDisplayColorPrimvar();
	//colPrimvar.Set();
	//colPrimvar.SetIndices();
	//colPrimvar.SetInterpolation(UsdGeomTokens->vertex);

	VtArray<float> opacity;
	opacity.push_back(1);
	meshPrim.CreateDisplayOpacityAttr().Set(opacity);

	//attr = meshPrim.AddTransformOp();
	//attr.Set(transform);

	//create and set custom attr
	//meshPrim.CreatePrimvar(TfToken("HE_Halfedges"), SdfValueTypeNames->Int4Array).Set(hes);
	//meshPrim.CreatePrimvar(TfToken("HE_Vertices"), SdfValueTypeNames->IntArray).Set(vertices);
	//meshPrim.CreatePrimvar(TfToken("HE_VertexColors"), SdfValueTypeNames->Float3Array).Set(vCols);
	//meshPrim.CreatePrimvar(TfToken("HE_Faces"), SdfValueTypeNames->IntArray).Set(faces);
	//meshPrim.CreatePrimvar(TfToken("HE_FaceNormals"), SdfValueTypeNames->Double3Array).Set(faceNormals);
	//meshPrim.CreatePrimvar(TfToken("HE_FaceColors"), SdfValueTypeNames->Double3Array).Set(faceColors);
...
void AddScopeProperty(UsdStageRefPtr& stage, UsdGeomScope& scope, TfToken& token, SdfValueTypeName typeName, const T& propertyValue)
{

}

template <typename T>
void AddProperty(UsdGeomMesh& meshPrim, TfToken& token, SdfValueTypeName typeName, const T& propertyValue)
{
	meshPrim.CreatePrimvar(token, typeName);

	UsdAttribute attr;
	attr = meshPrim.GetPrimvar(token);
	attr.Set(propertyValue);
}
...
void update(int value)
{
	if (compute)
	{
		...
		zObjMesh oMesh;
		zFnMesh fnMesh(oMesh);
		//fnMesh.from(folderPath + "panels.obj", zOBJ);
		//fnMesh.from(folderPath + "colorCube.json", zJSON);
		fnMesh.from(folderPath + "roofData_20230905.json", zJSON); 

		int numMeshes = fnMesh.numPolygons();
		//numMeshes = 5;
		zObjMeshArray outMeshArr;
		outMeshArr.assign(numMeshes, zObjMesh());

		readData(folderPath + "data.csv", dataPoints);

		for (int i = 0; i < numMeshes; i++)
		{
			zFnMesh fn(outMeshArr[i]);

			zPointArray pos;
			zIntArray pCounts;
			zIntArray pConnects;

			zItMeshFace f(oMesh, i);
			f.getVertexPositions(pos);
			pCounts.push_back(4);
			pConnects = { 0,1,2,3 };

			fn.create(pos, pCounts, pConnects);
			fn.setVertexColor(zColor(1, 1, 1, 1), true);
		}
		...
		cout << endl;
		cout << dataPoints.rows() << "," << dataPoints.cols() << endl;

		// add scope
		string scope_root;
		scope_root = s_root + "/Clusters";
		UsdGeomScope clusters = UsdGeomScope::Define(stage_out,SdfPath(scope_root));
		//AddScopeProperty(stage_out, clusters);

		// add all meshes to the usd file
		for (int i = 0; i < numMeshes; i++)
		{
			UsdGeomMesh meshPrim = UsdGeomMesh::Define(stage_out, SdfPath(s_prim + to_string(i)));

			// Convert zMesh to usd mesh
			int clusterID = dataPoints(i, 0);
			float planarity = dataPoints(i, 1);
			VtArray<float> clusterData;
			VtArray<float> clusterCentroid;

			clusterData.push_back(dataPoints(i, 2));
			clusterData.push_back(dataPoints(i, 3));
			clusterData.push_back(dataPoints(i, 4));
			clusterData.push_back(dataPoints(i, 5));
			clusterData.push_back(dataPoints(i, 6));
			clusterData.push_back(dataPoints(i, 7));
			clusterData.push_back(dataPoints(i, 8));

			clusterCentroid.push_back(dataPoints(i, 9));
			clusterCentroid.push_back(dataPoints(i, 10));
			clusterCentroid.push_back(dataPoints(i, 11));
			clusterCentroid.push_back(dataPoints(i, 12));
			clusterCentroid.push_back(dataPoints(i, 13));
			clusterCentroid.push_back(dataPoints(i, 14));
			clusterCentroid.push_back(dataPoints(i, 15));
			
			GfVec3f clusterColor (dataPoints(i, 16), dataPoints(i, 17), dataPoints(i, 18));


			//cout << clusterID << "," << planarity << endl;
			cout << i << "/" << numMeshes << endl;

			toUSDFile(meshPrim, outMeshArr[i]);
			AddProperty(meshPrim, TfToken("clusterID"), SdfValueTypeNames->Int, clusterID);
			AddProperty(meshPrim, TfToken("planarity"), SdfValueTypeNames->Float, planarity);
			AddProperty(meshPrim, TfToken("clusterData"), SdfValueTypeNames->FloatArray, clusterData);
			AddProperty(meshPrim, TfToken("clusterCentroid"), SdfValueTypeNames->FloatArray, clusterCentroid);
			AddProperty(meshPrim, TfToken("clusterColor"), SdfValueTypeNames->Color3f, clusterColor);
		}
