...
string dirPath = "data/graphPolyhedras/GFP";
vector<string> forceFiles;

string outPath = dirPath + "/out";
...
void setup()
{
	...
	formObj.setDisplayElements(true, true);

	for (int i = 0; i < forceObjs.size(); i++)	forceObjs[i].setDisplayElements(false, true, false);
	for (int i = 0; i < polytopalObjs.size(); i++)	polytopalObjs[i].setDisplayElements(false, true, true);
	...
	S.addSlider(&multiply, "multiply");
	S.sliders[2].attachToVariable(&multiply, 1, 10);

	S.addSlider(&offset, "offset");
	S.sliders[3].attachToVariable(&offset, 0.01, 1.00);
	
	S.addSlider(&angleTolerance, "angleTolerance");
	S.sliders[4].attachToVariable(&angleTolerance, 0.001, 20.00);

	S.addSlider(&vLoad, "vLoad");
	S.sliders[5].attachToVariable(&vLoad, -10.0, 10.0);
	...
	B.buttons[0].attachToVariable(&c_force);

	B.addButton(&c_form, "c_form");	
	B.buttons[1].attachToVariable(&c_form);
	...
	B.buttons[5].attachToVariable(&d_polytopal);

	B.addButton(&d_ForceDiagram, "d_ForceDiagram");
	B.buttons[6].attachToVariable(&d_ForceDiagram);

	B.addButton(&d_FormDiagram, "d_FormDiagram");	
	B.buttons[7].attachToVariable(&d_FormDiagram);

	B.addButton(&exportPolytopal, "e_polytopals");
	B.buttons[8].attachToVariable(&exportPolytopal);
...
void update(int value)
{
	...
	{
		forceObjs[i].setDisplayObject(d_ForceDiagram);
	}

	for (int i = 0; i < polytopalObjs.size(); i++)
	{
		polytopalObjs[i].setDisplayObject(d_polytopal);
	}
...
*/
	...
	if (equilibrium)
	{
		...
		if (out) myPolytopal.setDualEdgeWeightsfromPrimal(zDomainFloat(2.0, 10.0));
	...
	if (exportPolytopal)
	{
		...
		{
			string path1 = outPath + "/polytopals_" + to_string(i) + ".json";
			myPolytopal.fnPolytopals[i].to(path1, zJSON);
		}

		myPolytopal.fnForm.to(outPath + "/graph" + ".json", zJSON);
