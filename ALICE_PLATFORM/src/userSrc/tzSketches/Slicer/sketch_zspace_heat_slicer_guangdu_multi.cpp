...
string inPath = "data/Slicer/guangdu/in/";
string outPath = "data/Slicer/guangdu/out/";
...
void computeBlock()
{
	...
	chk = core.json_read(files_json[block_id], j);
...
void computeHeat()
{
	...
	if (!mode)
	{
		block.start_boundary_ids.insert(block.start_boundary_ids.end(), block.end_boundary_ids.begin(), block.end_boundary_ids.end());
		block.sliceMesh.computeGeodesics_Exact(block.start_boundary_ids, geodesics_start);
	}
	else
	{
		block.sliceMesh.computeGeodesics_Exact(block.start_boundary_ids, geodesics_start);
		block.sliceMesh.computeGeodesics_Exact(block.end_boundary_ids, geodesics_end);
	}
	...
	printf("\n totalGraphs %i ", block.totalGraphs);
...
void computeContour()
{
	...
	for (int i = 0; i < block.totalGraphs; i++)
	{
		if (!mode)
		{
			computeContours_oneSide(o_sliceMesh, geodesics_start, i, block.totalGraphs, block.o_contours);
		}
		else
...
void exportToFile()
{
	...
	//computeBlock();
	//computeHeat();
	//zFnMesh fn(*block.sliceMesh.getRawInMesh());
	//fn.to(folder + "mesh_" + to_string(block.id) + ".json", zJSON);
...
void setup()
{
	...
	S.addSlider(&mode, "mode");
	S.sliders[1].attachToVariable(&mode, 0, 1);

	S.addSlider(&block_id_slider, "block_id");
	S.sliders[2].attachToVariable(&block_id_slider, 0, numBlocks);
