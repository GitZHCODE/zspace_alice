...
void computeColorBoundaryVertices(zObjMesh* o_mesh, zColor& col_left, zColor& col_right, zIntArray& bIDs)
{
	bIDs.clear();
	zObjGraphArray boundaries;

	zItMeshHalfEdge he(*o_mesh);
	for (he.begin(); !he.end(); he++) {
		if (!he.onBoundary() && !he.getSym().onBoundary()) {
			if (he.getFace().getColor() == col_left && he.getSym().getFace().getColor() == col_right) {
				bIDs.push_back(he.getVertex().getId());

			}
			//cout << he.getFace().getColor().r << "," << he.getFace().getColor().g << "," << he.getFace().getColor().b << endl;

		}
	}

	cout << endl;
	cout << col_left.r << "," << col_left.g << "," << col_left.b << endl;
	cout << col_right.r << "," << col_right.g << "," << col_right.b << endl;
	...
	for (auto& o : bIDs) {
		cout << o << ",";
		model.displayUtils.drawPoint(pos[o], zColor(0, 0, 0, 1), 10);
	}
...
void setup()
{
	...
	//fnSliceMesh.from("data/HZ/hz_stadium_sliceMesh.json", zJSON);
	fnSliceMesh.from("data/Slicer/test.json", zJSON);

	// read mesh
		//myMeshParam.setFromFile("data/Parameterization/heatTest_ring_triangulated.obj", zOBJ);
	//myMeshParam.setFromFile("data/HZ/hz_stadium_sliceMesh.json", zJSON);
	myMeshParam.setFromFile("data/Slicer/test.json", zJSON);
...
void update(int value)
{
	if (compute_heat)
	{
		...
		//computeColorBoundaryVertices(o_inMesh, BLACK, GREEN, start_boundary_ids);
		//computeColorBoundaryVertices(o_inMesh, BLACK, BLUE, end_boundary_ids);
