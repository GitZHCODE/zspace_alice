...
string filePath = "data/polySplit/polylineData.json";
string path_field = "data/polySplit/densityMap.json";
...
void fieldFromMap(string& path, zObjMeshScalarField& oField, int resX, int resY)
{
	readMap = false;

	if (fs::exists(path_field))
	{
		readMap = true;
...
void setup()
{
	...
	fromFile(filePath, inPolys);

	fieldFromMap(path_field, oDensityField, 100, 100);
