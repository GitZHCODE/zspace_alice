...
public:
	...
	void create(string path)
	{
		...
		{
			if (v.getColor() == BLACK) fixedVerts.push_back(v.getId());
		}
		...
		// compute gaussian curvature once
				// Gaussian deviations
		for (zItMeshVertex v(oMesh); !v.end(); v++)
		{
			if (v.getGaussianCurvature() < tol_dev) v.setColor(zColor(0, 0, 1, 1));
			else v.setColor(zColor(1, 0, 0, 1));
		}
		fnDyMesh.computeFaceColorfromVertexColor();
	}
	...
	void computeCCF()
	{
		...
		for (int i = 0; i < numIterations; i++)
		{
			...
			{
				fnDyMesh.addRigidLineForce(0.5, 0.001, vertexAlignPairs, dists, devs_align, forceDir_align, exit_align);
			}
			...
			//	{
			//		zItMeshEdge e(oMesh, eIds[i]);
			//		if (e.getWeight() > 1.0)
			//			fnDyMesh.addLoadForce(0.01, v.getId(), zVector(0, 0, 1));
	...
	void draw(bool b_drawMesh = true, bool b_drawForces = true)
	{
		if (b_drawMesh)
		{
			drawMesh();
...
string fileName = "ccf";
string path = "data/ccf/" + fileName;
string path_out = "data/ccf/" + fileName;
...
void setup()
{
	...
	S.addSlider(&ccf.strength_planar, "str_planar");
	S.sliders[1].attachToVariable(&ccf.strength_planar, 0, 1);

	S.addSlider(&ccf.strength_dev, "str_dev");
	S.sliders[2].attachToVariable(&ccf.strength_dev, 0, 1);

	//S.addSlider(&strength_panelBoundaryplanar, "str_bplanar");
	//S.sliders[3].attachToVariable(&strength_panelBoundaryplanar, 0, 1);

	S.addSlider(&ccf.tol_unroll, "tol_unroll");
	S.sliders[3].attachToVariable(&ccf.tol_unroll, 0.000001, 0.01);

	S.addSlider(&ccf.tol_planar, "tol_planar");
	S.sliders[4].attachToVariable(&ccf.tol_planar, 0.0001, 0.01);

	S.addSlider(&ccf.tol_dev, "tol_dev");
	S.sliders[5].attachToVariable(&ccf.tol_dev, 0.0001, 0.01);
...
{
	ccf.oMesh_in.setDisplayObject(d_inMesh);
	ccf.oMesh.setDisplayObject(d_ccfMesh);
	ccf.oMesh_unroll.setDisplayObject(d_unrollMesh);
...
void draw()
{
	...
	drawString("planar deviation max:" + to_string(ccf.fPlanar_max), vec(winW - 350, winH - 900, 0));
	drawString("planar deviation min:" + to_string(ccf.fPlanar_min), vec(winW - 350, winH - 875, 0));

	drawString("gaussian deviation max:" + to_string(ccf.vGauss_max), vec(winW - 350, winH - 825, 0));
	drawString("gaussian deviation min:" + to_string(ccf.vGauss_min), vec(winW - 350, winH - 800, 0));


	drawString("b planar deviation max:" + to_string(ccf.bPlanar_max), vec(winW - 350, winH - 750, 0));
	drawString("b planar deviation min:" + to_string(ccf.bPlanar_min), vec(winW - 350, winH - 725, 0));
...
void keyPress(unsigned char k, int xm, int ym)
{
	...
	if (k == 'w')
	{
		if (ccf.bsfcurrentID < ccf.num_BSF) ccf.bsfcurrentID++;	
	}

	if (k == 'k')
	{
		// pull creases
		for (zItMeshVertex v(ccf.oMesh); !v.end(); v++)
		{
			zIntArray eIds;
			v.getConnectedEdges(eIds);
			for (int i = 0; i < eIds.size(); i++)
			{
				zItMeshEdge e(ccf.oMesh, eIds[i]);
				if (e.getWeight() > 1.0)
					v.getRawPosition()->z += 0.01;
	...
	if (k == 's')
	{
		if (ccf.bsfcurrentID > 0) ccf.bsfcurrentID--;
	}
}
