...
zDoubleArray vForceDensities, vMass;
...
void setup()
{
	...
	fdm_initialise(&vPositions[0], &pCounts[0], &pConnects[0], &triCounts[0], &triConnects[0],fnMesh.numVertices(), fnMesh.numPolygons());
	...
	fdm_setFixed(&fixedVertices[0], fixedVertices.size());
...
void update(int value)
{
	...
	{	
		...
		bool out = fdm_compute(&vForceDensities[0], &vMass[0], &out_vPositions[0]);
